...
extension CashDB {
    static func getAllExpenseData(sortBy: CashDBSort = .occuredOn,
                                  ascending: Bool = true,
                                  filterTime: CashDBFilterTime = .all) -> NSFetchRequest<CashDB> {
        if let request = CashDB.fetchRequest() as? NSFetchRequest<CashDB> {
